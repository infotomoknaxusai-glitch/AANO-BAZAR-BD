...
Output ONLY the style brief text — no labels, no quotes, no preamble.`;
  ...
  // AANO BAZAR AI Shopping Concierge Assistant
  app.post("/api/ai-assistant", async (req, res) => {
    try {
      const { message, history = [], currentProduct } = req.body;
      const ai = getAiClient();
      
      let contextInfo = `You are "AanoBot", the smart and courteous shopping assistant for AANO BAZAR - the ultimate multi-category superstore.
AANO BAZAR offers:
1. Grocery (Organic staples, Himalayan spices, fresh farm produce, gourmet beverages)
2. Electronics (Ultra laptops, ANC headphones, Smart watches, 4K cameras, Gaming consoles)
3. Fashion (Premium ethnic & western wear, artisan leather shoes, designer watches, eyewear)
4. Beauty (Korean skincare, luxury perfumes, organic botanical oils, hair serums)
5. Health & Wellness (Immunity boosters, whey protein, herbal ayurvedic teas, fitness gear)
6. Home & Kitchen (Cast iron cookware, barista espresso machines, smart robot vacuums, decor)
7. Mobile & Accessories (Flagship 5G phones, GaN fast chargers, magnetic cases, wireless earbuds)

Key Store Perks:
- 100% genuine guaranteed with official manufacturer warranty
- Express 2-hour grocery delivery and 24-hour nationwide standard delivery
- Tiered Cashback: up to 20% on select categories
- 15-day hassle-free return policy
- Dedicated dealer wholesale portal and seller marketplace.

Instructions:
- Give helpful, warm, concise, and accurate product advice, recommendations, comparison, and shopping tips.
- If relevant, mention specific AANO BAZAR perks (Flash deals, Cashback, Free delivery over $35/Rs 2000).
- If the user asks about orders, returns, selling, or wholesale, provide straightforward guidance and direct them to the appropriate tab in AANO BAZAR.`;

      if (currentProduct) {
        contextInfo += `\nThe user is currently viewing this product: "${currentProduct.name}" (Category: ${currentProduct.category}, Price: ${currentProduct.price}, Rating: ${currentProduct.rating}★).`;
      }
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          ...
          ...history.map((h: { sender: string; text: string }) => ({
            text: `${h.sender === 'user' ? 'Customer' : 'AanoBot'}: ${h.text}`
          })),
      ...
      res.json({ reply: response.text || "Hello! I am here to help you find the best products, deals, and answers on AANO BAZAR." });
    } catch (error: any) {
      console.error("AI Assistant error:", error);
      // Fallback friendly reply if API key is not configured or fails
      res.json({
        reply: "Welcome to AANO BAZAR! I can assist you with product recommendations across Grocery, Electronics, Fashion, Beauty, and Mobile Accessories, or guide you through our Flash Deals and Cashback offers."
      });
    }
  });
  ...
  app.post("/api/track-order", (req, res) => {
    ...
    res.json({
      orderId: cleanId || 'AB-892419',
      status: 'In Transit',
      carrier: 'AANO Express Logistics',
      estimatedDelivery: 'Tomorrow by 4:00 PM',
      steps: [
        { title: 'Order Confirmed', date: 'Yesterday, 10:24 AM', completed: true },
        { title: 'Packed & Quality Checked at AANO Central Hub', date: 'Today, 08:15 AM', completed: true },
        { title: 'Dispatched with Delivery Partner', date: 'Today, 02:40 PM', completed: true },
        { title: 'Out for Delivery', date: 'Expected Tomorrow, 09:00 AM', completed: false },
        { title: 'Delivered', date: 'Expected Tomorrow, 04:00 PM', completed: false }
      ]
    });
  });
